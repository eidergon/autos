...
Func NOTUS($username, $password)
	Run("cmd.exe /c start msedge --new-window --app=https://appsnotus.logytechmobile.com/notusils/Login.aspx?accion=cerrarSesion", "", @SW_HIDE)
	WinWaitActive("Notus ILS - Login")
	WinSetState("Notus ILS - Login", "", @SW_MAXIMIZE)
	MouseMove ( 2000, 4000, 0)
	If WinActive("Notus ILS - Login") Then
		Send($username) 
		MouseMove ( 2000, 4000, 0)
		sleep(1000)
		MouseMove ( 2000, 4000, 0)
...
If $CmdLine[0] > 0 Then
	...
	While 1
		...
		If Not $funcionEjecutada Then
			NOTUS($dataToSendUser,$dataToSendPass)
