...
Func MYAPPS($username, $password)
	Run("cmd.exe /c start msedge --new-window --app=https://myappsclaro.claro.com.co/index.php", "", @SW_HIDE)
	WinWaitActive("..::My Apps Claro::..")
	Sleep(1000)
	...
	Send("{TAB}")
	MouseMove ( 2000, 4000, 0)
	If WinActive("..::My Apps Claro::..") Then
		Send($username)
		MouseMove ( 2000, 4000, 0)
		Send("{TAB}")
		MouseMove ( 2000, 4000, 0)
...
If $CmdLine[0] > 0 Then
	...
	While 1
		...
		If Not $funcionEjecutada Then
			MYAPPS($dataToSendUser,$dataToSendPass)
